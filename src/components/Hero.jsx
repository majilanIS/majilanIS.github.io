...
/* ─── corner bracket SVG ────────────────────────────────────── */
function Corner({ pos, color, size = 28, thickness = 2.5 }) {
  ...
  const flip = {
    ...
    topLeft: "none",
  }[Object.keys(pos).includes("right") ? (Object.keys(pos).includes("top") ? "topRight" : "bottomRight") : (Object.keys(pos).includes("top") ? "topLeft" : "bottomLeft")];
  return (
    <svg style={{ ...s, transform: flip }} width={size} height={size} viewBox="0 0 28 28" fill="none">
      <path d={`M2 ${size} L2 2 L${size} 2`} stroke={color} strokeWidth={thickness} strokeLinecap="square" fill="none" />
    </svg>
  );
}
...
export default function Hero() {
  ...
  return (
    <div style={{ fontFamily: "'Sora', 'DM Sans', sans-serif", background: t.bg, minHeight: "100vh", transition: "background 0.4s" }}>
      {/* Google Fonts */}
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .nav-link {
          ...
          color: ${t.navIcon}; text-decoration: none; font-size: 14px; font-weight: 500;
          padding: 10px 0; border-left: 2px solid transparent;
          transition: color 0.2s, border-color 0.2s, padding-left 0.2s;
          cursor: pointer;
        }
        .nav-link:hover, .nav-link.active {
          color: ${t.accent}; border-left-color: ${t.accent}; padding-left: 10px;
        }
        .nav-link.active { color: ${t.accent}; }
        .btn-primary {
        ...
        .toggle-btn {
          ...
          background: ${t.toggleBg}; border: 1.5px solid ${t.border};
          color: ${t.text}; cursor: pointer; font-size: 18px;
          transition: background 0.3s, border-color 0.3s, transform 0.2s;
        }
        .toggle-btn:hover { transform: scale(1.1) rotate(15deg); border-color: ${t.accent}; }
        .stat-card {
      ...
      <aside style={{
        ...
        background: t.bgNav, borderRight: `1px solid ${t.border}`,
        display: "flex", flexDirection: "column", zIndex: 100,
        padding: "28px 0", transition: "background 0.4s, border-color 0.4s",
      }}>
        {/* Logo */}
        <div style={{ padding: "0 24px 28px", borderBottom: `1px solid ${t.border}` }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            <div style={{
              ...
              fontWeight: 700, fontSize: 17, color: "#fff", letterSpacing: "-0.02em",
            }}>C</div>
            <div>
              <div style={{ fontWeight: 800, fontSize: 15, color: t.text, letterSpacing: "-0.01em" }}>Chekole</div>
            </div>
          </div>
        </div>

        {/* Nav links */}
        <nav style={{ flex: 1, padding: "20px 24px", display: "flex", flexDirection: "column", gap: 2 }}>
          {[
            ...
            <a key={label} href={`#${label.toLowerCase().replace(" ", "-")}`}
              ...
              style={{ color: active ? t.accent : t.navIcon }}>
        ...
        <div style={{ padding: "20px 24px", borderTop: `1px solid ${t.border}` }}>
          <div style={{ fontSize: 11, color: t.textMuted, fontWeight: 600, letterSpacing: "0.05em", marginBottom: 10, textTransform: "uppercase" }}>Appearance</div>
          <div style={{ display: "flex", gap: 8 }}>
            <button
              className="toggle-btn"
              onClick={() => setTheme("dark")}
              title="Dark mode"
              style={{ background: theme === "dark" ? t.accentDim : t.toggleBg, borderColor: theme === "dark" ? t.accent : t.border }}
            >🌙</button>
            <button
              className="toggle-btn"
              onClick={() => setTheme("light")}
              title="Light mode"
              style={{ background: theme === "light" ? t.accentDim : t.toggleBg, borderColor: theme === "light" ? t.accent : t.border }}
            >🔆</button>
          </div>
        </div>
      </aside>
      ...
      {/* ── Main content ── */}
      <main style={{ marginLeft: 220, minHeight: "100vh", display: "flex", alignItems: "center", padding: "40px 56px 40px 64px", position: "relative", overflow: "hidden" }}>

        {/* Background decoration */}
        <div style={{ position: "absolute", top: -80, right: 100, width: 400, height: 400, borderRadius: "50%", background: `radial-gradient(circle, ${t.accent}18 0%, transparent 68%)`, pointerEvents: "none" }} />
        <div style={{ position: "absolute", bottom: -60, left: 80, width: 260, height: 260, borderRadius: "50%", background: `radial-gradient(circle, ${t.accent}0F 0%, transparent 68%)`, pointerEvents: "none" }} />
        ...
        <div style={{
          ...
          <div>
            ...
            <div style={fadeIn(0.12)}>
              <p style={{ fontSize: 15, color: t.textMuted, fontWeight: 400, marginBottom: "0.3rem" }}>
                Hello, my name is <span style={{ color: t.text, fontWeight: 600 }}>Chekole Ngusalem</span>
              </p>
              <h1 style={{
                ...
                I'm a{" "}
                <TypewriterRole accent={t.accent} />
              </h1>
            </div>

            {/* Divider line */}
            <div style={{ ...fadeIn(0.18), width: 48, height: 3, background: t.accent, borderRadius: 3, margin: "1rem 0 1.1rem" }} />

            {/* Lead */}
            <p style={{
              ...fadeIn(0.22),
              fontSize: 14.5, lineHeight: 1.72, color: t.textSub,
              maxWidth: "54ch", marginBottom: "1.5rem",
            }}>
              I build <span style={{ color: t.text, fontWeight: 600 }}>scalable backend systems</span>, RESTful APIs, and{" "}
              <span style={{ color: t.text, fontWeight: 600 }}>AI-powered applications</span>. Backend-focused, but fluent across the full stack — from Node.js services to React frontends and RAG pipelines.
            </p>

            {/* CTA buttons */}
            <div style={{ ...fadeIn(0.28), display: "flex", gap: 10, flexWrap: "wrap", marginBottom: "1.4rem" }}>
            ...
            <div style={{ ...fadeIn(0.34), display: "flex", gap: 8 }}>
              {[
                ...
                { label: "LinkedIn", icon: <svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor"><path d="M4.98 3.5C4.98 4.88 3.86 6 2.48 6S0 4.88 0 3.5 1.12 1 2.5 1s2.48 1.12 2.48 2.5zM.5 24h4V7h-4v17zM8.5 7v17h4v-9.3c0-2.3.8-3.9 2.9-3.9 2 0 2.1 1.8 2.1 4v9.2h4V13c0-5.6-3-8.2-7-8.2-3.2 0-4.6 1.8-5.1 3.1V7h-1z"/></svg> },
                { label: "X", icon: <svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor"><path d="M23 3.01a10.9 10.9 0 0 1-3.14.86A4.93 4.93 0 0 0 22.4.36a9.86 9.86 0 0 1-3.13 1.2 4.92 4.92 0 0 0-8.39 4.48A13.96 13.96 0 0 1 1.64.89 4.92 4.92 0 0 0 3.2 6.7a4.9 4.9 0 0 1-2.23-.62v.06a4.92 4.92 0 0 0 3.95 4.82 4.9 4.9 0 0 1-2.22.08 4.92 4.92 0 0 0 4.6 3.42A9.86 9.86 0 0 1 0 19.54a13.94 13.94 0 0 0 7.55 2.21c9.05 0 14-7.5 14-14v-.64A10.02 10.02 0 0 0 23 3.01z"/></svg> },
              ].map(({ label, icon }) => (
                <a key={label} href="#" className="social-btn" aria-label={label} style={{ color: t.textMuted, borderColor: t.border }}>
                  {icon}
                </a>
              ))}
            </div>
          ...
          {/* ── RIGHT: Portrait ── */}
          <div style={{ ...fadeIn(0.1), display: "flex", flexDirection: "column", alignItems: "center", gap: 14 }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 8, width: "100%" }}>
              {[
                ...
                <div key={label} className="stat-card" style={{ background: t.bgCard, borderColor: t.border }}>
                  <div style={{ fontSize: 18, fontWeight: 800, color: t.accent, lineHeight: 1, marginBottom: 3 }}>{num}</div>
                  <div style={{ fontSize: 10, fontWeight: 600, color: t.textMuted, letterSpacing: "0.04em", textTransform: "uppercase" }}>{label}</div>
                </div>
              ))}
            </div>
          </div>
      ...
      <style>{`
        @media (max-width: 860px) {
          ...
          main { margin-left: 0 !important; padding: 28px 20px !important; }
